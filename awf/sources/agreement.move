...
//         ctx: &mut TxContext
//     ) {
//         let mut parties = sui::vec_map::empty<address, bool>();
//         let mut num = 0;
//         while (num < vector::length(&party_addresses)) {
//             sui::vec_map::insert(&mut parties, *vector::borrow(&party_addresses, num), false);
//             num = num + 1;
//         };
...
//             playback_data,
//             zka_verified: false,
//             verifiers: sui::vec_map::empty(),
//         };
//         let agreement_id = object::uid_to_inner(&agreement.id);
//         sui::event::emit(AgreementCreated { agreement_id, creator: tx_context::sender(ctx) });
...
//         assert!(sui::vec_map::contains(&agreement.parties, &sender), 100);
...
//         let sender = tx_context::sender(ctx);
//         assert!(sui::vec_map::contains(&agreement.parties, &sender), 100);
//         sui::vec_map::insert(&mut agreement.verifiers, verifier, true);
//     }

//     // Access playback data
//     public fun get_playback_data(agreement: &Agreement, requester: address): vector<u8> {
//         assert!(sui::vec_map::contains(&agreement.verifiers, &requester), 101);
